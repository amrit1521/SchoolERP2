...
const PHomeWork = () => {
  ...
  const [allChildHomeWork, setAllChildHomeWork] = useState<any[]>([]);
  ...
  const fetchHomeWorks = async (userId: number) => {
    ...
    if (userId) {
      try {
        const { data } = await getAllMyChildHomeWork(userId);
        ...
        if (data.success) {
          ...
          setAllChildHomeWork(data.data);
          setHomeworks(data.data[0]?.home_work);
        }
      } catch (error) {
  ...
  const handleApplyClick = () => {
    if (filter?.studentId) {
      setHomeworks(
        allChildHomeWork.filter(
          (work: any) => work.student_id == filter?.studentId
        )[0]?.home_work
      );
    }
