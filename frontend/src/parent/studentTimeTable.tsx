...
const PStudentClassTimetable = () => {
  ...
  const [allChildTimeTable, setAllChildTimeTable] = useState<any[]>([]);
  ...
  const fetchTimeTable = async (userId: number) => {
    setLoading(true);
    try {
      const { data } = await getAllChildTimeTable(userId);
      if (data.success) {
        setStudentOptions(
        ...
        setAllChildTimeTable(data.data);
        setTimeTable(data.data[0].timetable);
      }
    } catch (error: any) {
      console.log(error);
  ...
  const handleApplyClick = async (e: React.FormEvent) => {
    ...
    try {
      if (filter?.studentId) {
        setTimeTable(
          allChildTimeTable.filter(
            (subject: any) => subject.student_id == filter?.studentId
          )[0]?.timetable
        );
      }
  ...
  const handleResetFilter = (e?: React.MouseEvent) => {
    ...
    setTimeTable(allChildTimeTable[0]?.timetable);
    if (dropdownMenuRef.current) {
      dropdownMenuRef.current.classList.remove("show");
    }
  };
    ...
    if (roleId) {
      ...
      if (data.success) {
        const currentPermission = data.result
          .filter((perm: any) => perm?.module_name === "TimeTable")
          .map((perm: any) => ({
...
export default PStudentClassTimetable;
