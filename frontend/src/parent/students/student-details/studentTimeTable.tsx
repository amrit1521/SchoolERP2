...
const PStudentTimeTable = () => {
  ...
  useEffect(() => {
    ...
    const fetchStudentAndTimeTable = async () => {
      ...
      try {
        const [studentRes, timetableRes] = await Promise.allSettled([
          specificStudentData1(Number(rollnum)),
          getStuTimeTable(Number(rollnum)),
        ]);
    ...
    if (rollnum) fetchStudentAndTimeTable();
  }, [rollnum]);
                          ...
                          >
                            ...
                            {loading ? ( // 👈 jab tak loading true hai skeleton dikhega
                              [...Array(2)].map((_, i) => (
                                <div
                                  key={i}
                                  className="bg-transparent-success rounded p-3 mb-4"
                                >
                                  <p className="mb-2">
...
export default PStudentTimeTable;
