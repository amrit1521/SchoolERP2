...
import { count, typetheory } from "../core/common/selectoption/selectoption";
...
const PStudentClassSubject = () => {
  ...
  const [allSubjectForchild, setAllSubjectForChild] = useState<any[]>([]);
  ...
  const fetchAllSubject = async (userId: any) => {
    ...
    try {
      const { data } = await getAllSubjectForClass(userId);
      if (data.success) {
        setStudentOptions(
        ...
        setAllSubjectForChild(data.data);
        setAllSubject(data.data[0]?.subjects);
      }
    } catch (error: any) {
      console.log(error);
...
export default PStudentClassSubject;
