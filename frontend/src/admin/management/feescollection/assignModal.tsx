...
import { allFeesMaster, allStudents, feesAssignToStudents, filterStudents, Imageurl } from '../../../service/api'
...
const AssignModal = ({ addModal, setAddModal, editModal, setEditModal }: any) => {
  ...
  const [orignalStudent, setOriginalStudent] = useState<any>([])
  ...
  const fetchAllMaterFees = async () => {
    ...
    try {
      ...
      if (response2?.data?.success) {
        ...
        setOriginalStudent(response2.data.students)
      } else {
  ...
  const handleAssignFees = async () => {
    ...
    if (payload.length === 0 || selectedFees.length === 0) {
      toast.error("Please select students and fees both  before assigning!");
      return;
    }
