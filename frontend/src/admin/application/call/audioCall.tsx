...
import { useState } from "react";
import AudioCallprops from "./audioCallProps";

const AudioCall = () => {

  const [joined, setJoined] = useState(false);
  const [roomID, setRoomID] = useState("school_audio_test");
  ...
  const handleJoin = () => {
    ...
    // Optional validation (2–20 chars, letters/numbers/_)
    const validName = /^[\u4e00-\u9fa5_a-zA-Z0-9]{2,20}$/;
    if (!validName.test(userName)) {
      alert("Invalid name: 2–20 letters/numbers/_ only");
      return;
    }
    setJoined(true);
  };
  ...
  if (joined) {
    return < AudioCallprops roomID={roomID} userName={userName} />;
  }
