...
interface Props {
  ...
  show: boolean;
  onClose: () => void;
  onAddMembers: (members: number[]) => void;
...
const AddMembersModal: React.FC<Props> = ({
  ...
  const toggleUser = (id: number) => {
    if (id === currentUserId) return; // current user can't add themselves
  ...
  const handleAddMembers = () => {
    ...
    // send selected member IDs to parent/component that calls API
      ...
      <div className="modal-dialog modal-dialog-centered modal-lg">
        <div className="modal-content shadow-lg rounded-4 border-0">
          ...
          <div className="modal-header bg-light border-bottom-0">
            <h4 className="modal-title fw-bold">Add Members to Group</h4>
          ...
          <div className="modal-body">
            ...
            {/* MEMBERS LIST */}
            <h5 className="fw-bold mb-3">Select Members to Add</h5>
              ...
              {filteredUsers.map((u) => (
                ...
                >
                  ...
                  <div className="flex-grow-1">
                    <h6 className="mb-0 text-capitalize fw-semibold">{u.name}</h6>
                    <small className="text-capitalize">{u.role_name}</small>
                  </div>
                </div>
              ))}
            </div>
